...
// Nova_x-ui logger must be initialised once before any code path that can
...
func setupIntegrationDB(t *testing.T) {
	...
	if err := database.InitDB(filepath.Join(dbDir, "Nova_x-ui.db")); err != nil {
...
func TestUpdateInboundClientIps_LiveIpNotBannedByStillFreshHistoricals(t *testing.T) {
	...
	// Nova_x-ui-iplimit.log must not contain a ban line.
	if info, err := os.Stat(readIpLimitLogPath()); err == nil && info.Size() > 0 {
		body, _ := os.ReadFile(readIpLimitLogPath())
		t.Fatalf("Nova_x-ui-iplimit.log should be empty when no ips are banned, got:\n%s", body)
	}
}
...
func TestUpdateInboundClientIps_ExcessLiveIpIsStillBanned(t *testing.T) {
	...
	// Nova_x-ui-iplimit.log must contain the ban line in the exact fail2ban format.
	body, err := os.ReadFile(readIpLimitLogPath())
	if err != nil {
		t.Fatalf("read Nova_x-ui-iplimit.log: %v", err)
	}
	wantSubstr := "[LIMIT_IP] Email = pr4091-abuse || Disconnecting OLD IP = 192.0.2.9"
	if !contains(string(body), wantSubstr) {
		t.Fatalf("Nova_x-ui-iplimit.log missing expected ban line %q\nfull log:\n%s", wantSubstr, body)
	}
}

// readIpLimitLogPath reads the Nova_x-ui-iplimit.log path the same way the job
...
func readIpLimitLogPath() string {
	...
	return filepath.Join(folder, "Nova_x-ui-iplimit.log")
}
