...
// GetUpdateInfo checks GitHub for the latest Nova_x-ui release.
...
func (s *PanelService) StartUpdate() error {
	...
	updateScript := fmt.Sprintf("set -o pipefail; %s -fLs https://raw.githubusercontent.com/CatMsg/Nova_x-ui/main/update.sh | %s", shellQuote(curl), shellQuote(bash))

	if systemdRun, err := exec.LookPath("systemd-run"); err == nil {
		unitName := fmt.Sprintf("Nova_x-ui-web-update-%d", time.Now().Unix())
...
func fetchLatestPanelVersion() (string, error) {
	...
	resp, err := client.Get("https://api.github.com/repos/CatMsg/Nova_x-ui/releases/latest")
	if err != nil {
		return "", err
	}
...
func resolveUpdateFolders() (string, string) {
	...
	if mainFolder == "" {
		mainFolder = "/usr/local/Nova_x-ui"
	}
