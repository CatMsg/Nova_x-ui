// Package service provides business logic services for the Nova_x-ui web panel,
...
import (
	...
	"github.com/CatMsg/Nova_x-ui/v3/database"
	"github.com/CatMsg/Nova_x-ui/v3/database/model"
	"github.com/CatMsg/Nova_x-ui/v3/logger"
	"github.com/CatMsg/Nova_x-ui/v3/util/common"
	"github.com/CatMsg/Nova_x-ui/v3/web/runtime"
	"github.com/CatMsg/Nova_x-ui/v3/xray"
...
func (s *InboundService) GetClients(inbound *model.Inbound) ([]model.Client, error) {
	...
	decodeClients := func(raw json.RawMessage) ([]model.Client, error) {
		if len(raw) == 0 {
			return nil, nil
		}
	...
	if raw, ok := settings["accounts"]; ok {
		clients, err := decodeClients(raw)
		if err != nil {
			return nil, err
		}
		...
		case "http", "socks", "mixed":
			for i := range clients {
				if clients[i].Email == "" && clients[i].User != "" {
...
func inboundClientSettingsKey(protocol model.Protocol) string {
	switch protocol {
...
func (s *InboundService) getAllEmails() ([]string, error) {
	...
	err := db.Raw(`
		SELECT DISTINCT email FROM (
			...
			UNION ALL
			SELECT JSON_EXTRACT(account.value, '$.email') AS email
			FROM inbounds,
				JSON_EACH(COALESCE(JSON_EXTRACT(inbounds.settings, '$.accounts'), '[]')) AS account
		)
		`).Scan(&emails).Error
	if err != nil {
		return nil, err
...
func (s *InboundService) getAllEmailSubIDs() (map[string]string, error) {
	...
	err := db.Raw(`
		SELECT email, sub_id FROM (
			...
			UNION ALL
			SELECT JSON_EXTRACT(account.value, '$.email') AS email,
			       JSON_EXTRACT(account.value, '$.subId') AS sub_id
			FROM inbounds,
				JSON_EACH(COALESCE(JSON_EXTRACT(inbounds.settings, '$.accounts'), '[]')) AS account
		)
		`).Scan(&rows).Error
...
func (s *InboundService) emailUsedByOtherInbounds(email string, exceptInboundId int) (bool, error) {
	...
	err := db.Raw(`
		SELECT COUNT(*) FROM (
			SELECT inbounds.id AS inbound_id, JSON_EXTRACT(client.value, '$.email') AS email
			FROM inbounds,
				JSON_EACH(COALESCE(JSON_EXTRACT(inbounds.settings, '$.clients'), '[]')) AS client
			UNION ALL
			SELECT inbounds.id AS inbound_id, JSON_EXTRACT(account.value, '$.email') AS email
			FROM inbounds,
				JSON_EACH(COALESCE(JSON_EXTRACT(inbounds.settings, '$.accounts'), '[]')) AS account
		)
		WHERE inbound_id != ?
...
func (s *InboundService) AddInbound(inbound *model.Inbound) (*model.Inbound, bool, error) {
	...
	if len(clients) > 0 {
		...
		if err2 := json.Unmarshal([]byte(inbound.Settings), &settings); err2 == nil && settings != nil {
			...
			settings[inboundClientSettingsKey(inbound.Protocol)] = updatedClients
...
func (s *InboundService) DelInbound(id int) (bool, error) {
	...
	if err = s.syncHy2PortForwarding(inbound, nil); err != nil {
		return false, err
	}
	clients, err := s.GetClients(inbound)
	if err != nil {
...
func (s *InboundService) SetInboundEnable(id int, enable bool) (bool, error) {
	...
	if err := s.syncHy2PortForwarding(inbound, inbound); err != nil {
...
func (s *InboundService) UpdateInbound(inbound *model.Inbound) (*model.Inbound, bool, error) {
	...
	tag := oldInbound.Tag
	oldInboundBeforeUpdate := *oldInbound
	...
	// Ensure created_at and updated_at exist in inbound.Settings clients
	{
		...
		oldKey := inboundClientSettingsKey(oldInbound.Protocol)
		newKey := inboundClientSettingsKey(inbound.Protocol)
		emailToCreated := map[string]int64{}
		emailToUpdated := map[string]int64{}
		if oldSettings != nil {
			if oc, ok := oldSettings[oldKey].([]any); ok {
				for _, it := range oc {
		...
		if err2 := json.Unmarshal([]byte(inbound.Settings), &newSettings); err2 == nil && newSettings != nil {
			now := time.Now().Unix() * 1000
			if nSlice, ok := newSettings[newKey].([]any); ok {
				for i := range nSlice {
	...
	if err = s.syncHy2PortForwarding(&oldInboundBeforeUpdate, oldInbound); err != nil {
		return inbound, needRestart, err
	}
...
func (s *InboundService) buildRuntimeInboundForAPI(tx *gorm.DB, inbound *model.Inbound) (*model.Inbound, error) {
	...
	clients, ok := settings[clientKey].([]any)
	if !ok && clientKey == "accounts" {
		clients, ok = settings["clients"].([]any)
	}
	if !ok {
		clients, ok = settings["accounts"].([]any)
	}
	if !ok {
...
func (s *InboundService) AddInboundClient(data *model.Inbound) (bool, error) {
	...
	settingsKey := inboundClientSettingsKey(oldInbound.Protocol)
	...
	for i := range clients {
		if (oldInbound.Protocol == model.HTTP || oldInbound.Protocol == model.Socks || oldInbound.Protocol == model.Mixed) &&
			clients[i].Email == "" && clients[i].User != "" {
			clients[i].Email = clients[i].User
		}
	}
	...
	interfaceClients, ok := settings[settingsKey].([]any)
	if !ok {
	...
	oldClients, ok := oldSettings[settingsKey].([]any)
	if !ok {
		oldClients = []any{}
	}
	oldClients = append(oldClients, interfaceClients...)

	oldSettings[settingsKey] = oldClients
...
func (s *InboundService) writeBackClientSubID(sourceInboundID int, sourceProtocol model.Protocol, client model.Client, subID string) (bool, error) {
	...
	settingsBytes, err := json.Marshal(map[string][]model.Client{
		inboundClientSettingsKey(sourceProtocol): {client},
	})
	if err != nil {
		return false, err
	}
...
func (s *InboundService) buildTargetClientFromSource(source model.Client, targetProtocol model.Protocol, email string, flow string) (model.Client, error) {
	...
	case model.HTTP, model.Socks, model.Mixed:
		target.User = s.generateRandomCredential(targetProtocol)
		target.Pass = s.generateRandomCredential(targetProtocol)
	case model.Hysteria:
...
func (s *InboundService) CopyInboundClients(targetInboundID int, sourceInboundID int, clientEmails []string, flow string) (*CopyClientsResult, bool, error) {
	...
	settingsPayload, err := json.Marshal(map[string][]model.Client{
		inboundClientSettingsKey(targetInbound.Protocol): newClients,
	})
	if err != nil {
...
func (s *InboundService) DelInboundClient(inboundId int, clientId string) (bool, error) {
	...
	for _, client := range interfaceClients {
		...
		if c_id == clientId {
			clientFound = true
			email, _ = c["email"].(string)
			if email == "" && (oldInbound.Protocol == model.HTTP || oldInbound.Protocol == model.Socks || oldInbound.Protocol == model.Mixed) {
	...
	settings[settingsKey] = newClients
...
func (s *InboundService) UpdateInboundClient(data *model.Inbound, clientId string) (bool, error) {
	...
	if (oldInbound.Protocol == model.HTTP || oldInbound.Protocol == model.Socks || oldInbound.Protocol == model.Mixed) &&
		clients[0].Email == "" && clients[0].User != "" {
		clients[0].Email = clients[0].User
	}
	...
	for index, oldClient := range oldClients {
		...
		case "http", "socks", "mixed":
			oldClientId = oldClient.User
			newClientId = clients[0].User
		case "hysteria", "hysteria2":
	...
	settingsClients, ok := oldSettings[settingsKey].([]any)
	if !ok {
		settingsClients = []any{}
	}
	...
	oldSettings[settingsKey] = settingsClients
...
func (s *InboundService) disableInvalidClients(tx *gorm.DB) (bool, int64, error) {
	...
	// get cascaded; older data where two identities reuse the same email
