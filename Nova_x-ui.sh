...
mkdir -p "${log_folder}"
iplimit_log_path="${log_folder}/Nova_x-ui-iplimit.log"
iplimit_banned_log_path="${log_folder}/Nova_x-ui-iplimit-banned.log"
...
install() {
    bash <(curl -Ls https://raw.githubusercontent.com/CatMsg/Nova_x-ui/main/install.sh)
    if [[ $? == 0 ]]; then
        if [[ $# == 0 ]]; then
            start
        else
...
update() {
    confirm "This function will update all Nova_x-ui components to the latest version, and the data will not be lost. Do you want to continue?" "y"
    if [[ $? != 0 ]]; then
        LOGE "Cancelled"
        if [[ $# == 0 ]]; then
            before_show_menu
    ...
    bash <(curl -Ls https://raw.githubusercontent.com/CatMsg/Nova_x-ui/main/update.sh)
    if [[ $? == 0 ]]; then
...
update_menu() {
    ...
    curl -fLRo /usr/bin/Nova_x-ui https://raw.githubusercontent.com/CatMsg/Nova_x-ui/main/Nova_x-ui.sh
    chmod +x ${panel_folder}/Nova_x-ui.sh
...
cleanup_hy2_forwarding_rules() {
    local hy2_script="${panel_folder}/scripts/hy2-forward.sh"
    if [[ -x "${hy2_script}" ]]; then
        echo -e "${green}Removing HY2 port forwarding rules...${plain}"
        bash "${hy2_script}" purge >/dev/null 2>&1 || true
        return 0
    fi

    if [[ -x "scripts/hy2-forward.sh" ]]; then
        echo -e "${green}Removing HY2 port forwarding rules...${plain}"
        bash "scripts/hy2-forward.sh" purge >/dev/null 2>&1 || true
    fi
}
...
uninstall() {
    ...
    if [[ $release == "alpine" ]]; then
        ...
        rc-update del Nova_x-ui
        rm /etc/init.d/Nova_x-ui -f
    else
        systemctl stop Nova_x-ui
        systemctl disable Nova_x-ui
        rm ${service_folder}/Nova_x-ui.service -f
    ...
    echo -e "${green}bash <(curl -Ls https://raw.githubusercontent.com/CatMsg/Nova_x-ui/master/install.sh)${plain}"
    echo ""
...
reset_user() {
    ...
    if [[ $twoFactorConfirm != "y" && $twoFactorConfirm != "Y" ]]; then
        ${panel_folder}/Nova_x-ui setting -username "${config_account}" -password "${config_password}" -resetTwoFactor false > /dev/null 2>&1
    else
        ${panel_folder}/Nova_x-ui setting -username "${config_account}" -password "${config_password}" -resetTwoFactor true > /dev/null 2>&1
        echo -e "Two factor authentication has been disabled."
    fi

    echo -e "Panel login username has been reset to: ${green} ${config_account} ${plain}"
    echo -e "Panel login password has been reset to: ${green} ${config_password} ${plain}"
    echo -e "${green} Please use the new login username and password to access the Nova_x-ui panel. Also remember them! ${plain}"
    confirm_restart
}
...
reset_webbasepath() {
    ...
    ${panel_folder}/Nova_x-ui setting -webBasePath "${config_webBasePath}" > /dev/null 2>&1
...
reset_config() {
    ...
    ${panel_folder}/Nova_x-ui setting -reset
    echo -e "All panel settings have been reset to default."
    restart
}

check_config() {
    local info=$(${panel_folder}/Nova_x-ui setting -show true)
    if [[ $? != 0 ]]; then
...
set_port() {
    ...
    if [[ -z "${port}" ]]; then
        ...
        ${panel_folder}/Nova_x-ui setting -port ${port}
...
start() {
    ...
    if [[ $? == 0 ]]; then
        ...
        check_status
        if [[ $? == 0 ]]; then
            LOGI "Nova_x-ui Started Successfully"
        else
...
stop() {
    ...
    if [[ $? == 1 ]]; then
        ...
        check_status
        if [[ $? == 1 ]]; then
            LOGI "Nova_x-ui and xray stopped successfully"
        else
...
restart() {
    if [[ $release == "alpine" ]]; then
        rc-service Nova_x-ui restart
    else
        systemctl restart Nova_x-ui
    fi
    sleep 2
    check_status
    if [[ $? == 0 ]]; then
        LOGI "Nova_x-ui and xray Restarted successfully"
    else
...
restart_xray() {
    if [[ $release == "alpine" ]]; then
        rc-service Nova_x-ui reload
    else
        systemctl reload Nova_x-ui
    fi
...
status() {
    if [[ $release == "alpine" ]]; then
        rc-service Nova_x-ui status
    else
        systemctl status Nova_x-ui -l
    fi
    if [[ $# == 0 ]]; then
        before_show_menu
    fi
...
enable() {
    if [[ $release == "alpine" ]]; then
        rc-update add Nova_x-ui default
    else
        systemctl enable Nova_x-ui
    fi
    if [[ $? == 0 ]]; then
        LOGI "Nova_x-ui Set to boot automatically on startup successfully"
    else
        LOGE "Nova_x-ui Failed to set Autostart"
    fi
...
disable() {
    ...
    if [[ $? == 0 ]]; then
        LOGI "Nova_x-ui Autostart Cancelled successfully"
    else
        LOGE "Nova_x-ui Failed to cancel autostart"
    fi
    ...
    if [[ $release == "alpine" ]]; then
        ...
        case "$choice" in
            0)
                ...
                ;;
            1)
                grep -F 'Nova_x-ui[' /var/log/messages
                if [[ $# == 0 ]]; then
                    before_show_menu
                fi
                ;;
        ...
        case "$choice" in
            0)
                ...
                ;;
            1)
                journalctl -u Nova_x-ui -e --no-pager -f -p debug
                if [[ $# == 0 ]]; then
                    before_show_menu
                fi
                ;;
...
disable_bbr() {
    ...
    if [ -f "/etc/sysctl.d/99-bbr-Nova_x-ui.conf" ]; then
        old_settings=$(head -1 /etc/sysctl.d/99-bbr-Nova_x-ui.conf | tr -d '#')
        sysctl -w net.core.default_qdisc="${old_settings%:*}"
        sysctl -w net.ipv4.tcp_congestion_control="${old_settings#*:}"
        rm /etc/sysctl.d/99-bbr-Nova_x-ui.conf
        sysctl --system
    else
...
enable_bbr() {
    ...
    if [ -d "/etc/sysctl.d/" ]; then
        ...
        } > "/etc/sysctl.d/99-bbr-Nova_x-ui.conf"
        if [ -f "/etc/sysctl.conf" ]; then
...
update_shell() {
    curl -fLRo /usr/bin/Nova_x-ui -z /usr/bin/Nova_x-ui https://github.com/CatMsg/Nova_x-ui/raw/main/Nova_x-ui.sh
    if [[ $? != 0 ]]; then
        echo ""
...
check_status() {
    if [[ $release == "alpine" ]]; then
        ...
        if [[ $(rc-service Nova_x-ui status | grep -F 'status: started' -c) == 1 ]]; then
            return 0
        else
            return 1
        fi
        ...
        temp=$(systemctl status Nova_x-ui | grep Active | awk '{print $3}' | cut -d "(" -f2 | cut -d ")" -f1)
...
check_enabled() {
    if [[ $release == "alpine" ]]; then
        if [[ $(rc-update show | grep -F 'Nova_x-ui' | grep default -c) == 1 ]]; then
            return 0
        else
            return 1
        fi
    else
        temp=$(systemctl is-enabled Nova_x-ui)
...
install_firewall() {
    ...
    if ufw status | grep -q "Status: active"; then
        ...
        ufw allow http
        ufw allow https
        ufw allow 2026/tcp #webPort
        ufw allow 2027/tcp #subport
...
open_ports() {
    ...
    read -rp "Enter the ports you want to open (e.g. 80,443,2026 or range 400-500): " ports

    # Check if the input is valid
    if ! [[ $ports =~ ^([0-9]+|[0-9]+-[0-9]+)(,([0-9]+|[0-9]+-[0-9]+))*$ ]]; then
        echo "Error: Invalid input. Please enter a comma-separated list of ports or a range of ports (e.g. 80,443,2026 or 400-500)." >&2
        exit 1
    fi
...
delete_ports() {
    ...
    if [[ $choice -eq 1 ]]; then
        ...
        # Deleting by ports
        read -rp "Enter the ports you want to delete (e.g. 80,443,2026 or range 400-500): " ports
...
update_geofiles() {
    ...
    for dat in "${dat_files[@]}"; do
        ...
        remote_file="${dat%%_*}"
        curl -fLRo ${panel_folder}/bin/${dat}.dat -z ${panel_folder}/bin/${dat}.dat \
...
ssl_cert_issue_for_ip() {
    ...
    local existing_webBasePath=$(${panel_folder}/Nova_x-ui setting -show true | grep -Eo 'webBasePath: .+' | awk '{print $2}')
    ...
    local reloadCmd="systemctl restart Nova_x-ui 2>/dev/null || rc-service Nova_x-ui restart 2>/dev/null"
...
ssl_cert_issue() {
    ...
    reloadCmd="Nova_x-ui restart"

    LOGI "Default --reloadcmd for ACME is: ${yellow}Nova_x-ui restart"
    LOGI "This command will run on every certificate issue and renew."
    read -rp "Would you like to modify --reloadcmd for ACME? (y/n): " setReloadcmd
    if [[ "$setReloadcmd" == "y" || "$setReloadcmd" == "Y" ]]; then
        echo -e "\n${green}\t1.${plain} Preset: systemctl reload nginx ; Nova_x-ui restart"
        echo -e "${green}\t2.${plain} Input your own command"
        echo -e "${green}\t0.${plain} Keep default reloadcmd"
        read -rp "Choose an option: " choice
        case "$choice" in
            1)
                LOGI "Reloadcmd is: systemctl reload nginx ; Nova_x-ui restart"
                reloadCmd="systemctl reload nginx ; Nova_x-ui restart"
                ;;
            2)
                LOGD "It's recommended to put Nova_x-ui restart at the end, so it won't raise an error if other services fails"
                read -rp "Please enter your reloadcmd (example: systemctl reload nginx ; Nova_x-ui restart): " reloadCmd
...
iplimit_main() {
    ...
    case "$choice" in
        ...
        3)
            ...
            if [[ $? == 0 ]]; then
                fail2ban-client reload --restart --unban Nova_x-ui-iplimit
        ...
        5)
            ...
            if [[ $ban_ip =~ $ipv4_regex || $ban_ip =~ $ipv6_regex ]]; then
                fail2ban-client set Nova_x-ui-iplimit banip "$ban_ip"
        ...
        6)
            ...
            if [[ $unban_ip =~ $ipv4_regex || $unban_ip =~ $ipv6_regex ]]; then
                fail2ban-client set Nova_x-ui-iplimit unbanip "$unban_ip"
...
install_iplimit() {
    if ! command -v fail2ban-client &> /dev/null; then
        ...
        # even though our own Nova_x-ui-iplimit jail uses iptables. Bundling the binary
...
remove_iplimit() {
    ...
    case "$num" in
        1)
            rm -f /etc/fail2ban/filter.d/Nova_x-ui-iplimit.conf
            rm -f /etc/fail2ban/action.d/Nova_x-ui-iplimit.conf
            rm -f /etc/fail2ban/jail.d/Nova_x-ui-iplimit.conf
            if [[ $release == "alpine" ]]; then
                rc-service fail2ban restart
            else
                systemctl restart fail2ban
...
show_banlog() {
    ...
    if [[ -f "$system_log" ]]; then
        ...
        grep "Nova_x-ui-iplimit" "$system_log" | grep -E "Ban|Unban" | tail -n 10 || echo -e "${yellow}No recent system ban activities found${plain}"
        echo ""
    fi

    if [[ -f "${iplimit_banned_log_path}" ]]; then
        echo -e "${green}Nova_x-ui IP limit ban log entries:${plain}"
    ...
    fail2ban-client status Nova_x-ui-iplimit || echo -e "${yellow}Unable to get jail status${plain}"
}
...
create_iplimit_jails() {
    ...
    cat << EOF > /etc/fail2ban/jail.d/Nova_x-ui-iplimit.conf
[Nova_x-ui-iplimit]
enabled=true
backend=auto
...
EOF
    ...
    cat << EOF > /etc/fail2ban/filter.d/Nova_x-ui-iplimit.conf
[Definition]
...
EOF
    ...
    cat << EOF > /etc/fail2ban/action.d/Nova_x-ui-iplimit.conf
[INCLUDES]
...
iplimit_remove_conflicts() {
    ...
    for file in "${jail_files[@]}"; do
        # Check for [Nova_x-ui-iplimit] config in jail file then remove it
        if test -f "${file}" && grep -qw 'Nova_x-ui-iplimit' ${file}; then
            sed -i "/\[Nova_x-ui-iplimit\]/,/^$/d" ${file}
            echo -e "${yellow}Removing conflicts of [Nova_x-ui-iplimit] in jail (${file})!${plain}\n"
        fi
    done
}
...
SSH_port_forwarding() {
    ...
    local existing_webBasePath=$(${panel_folder}/Nova_x-ui setting -show true | grep -Eo 'webBasePath: .+' | awk '{print $2}')
    local existing_port=$(${panel_folder}/Nova_x-ui setting -show true | grep -Eo 'port: .+' | awk '{print $2}')
    local existing_listenIP=$(${panel_folder}/Nova_x-ui setting -getListen true | grep -Eo 'listenIP: .+' | awk '{print $2}')
    local existing_cert=$(${panel_folder}/Nova_x-ui setting -getCert true | grep -Eo 'cert: .+' | awk '{print $2}')
    local existing_key=$(${panel_folder}/Nova_x-ui setting -getCert true | grep -Eo 'key: .+' | awk '{print $2}')
    ...
    case "$num" in
        1)
            if [[ -z "$existing_listenIP" || "$existing_listenIP" == "0.0.0.0" ]]; then
                ...
                ${panel_folder}/Nova_x-ui setting -listenIP "${config_listenIP}" > /dev/null 2>&1
            ...
            ;;
        2)
            ${panel_folder}/Nova_x-ui setting -listenIP 0.0.0.0 > /dev/null 2>&1
...
|  ${blue}Nova_x-ui restart-xray${plain}          - Restart Xray                     │
...
│  ${blue}Nova_x-ui update-all-geofiles${plain}   - Update all geo files             │
...
show_menu() {
    echo -e "
╔────────────────────────────────────────────────╗
│   ${green}Nova_x-ui Panel Management Script${plain}                │
