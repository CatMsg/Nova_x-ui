...
const BACKEND_TARGET = 'http://localhost:2026';

function resolveDBPath() {
  const envFolder = process.env.XUI_DB_FOLDER;
  if (envFolder) return path.join(envFolder, 'Nova_x-ui.db');
  const repoDB = path.resolve(__dirname, '..', 'Nova_x-ui.db');
  if (fs.existsSync(repoDB)) return repoDB;
  return '/etc/Nova_x-ui/Nova_x-ui.db';
}
...
function injectBasePathPlugin() {
  return {
    name: 'Nova_x-ui-inject-base-path',
    apply: 'serve',
