...
// === Static option lists (mirror previous) =============================
...
// Country / service lists mirror the Nova_x-ui panel's settingsData
// (web/html/xray.html on main). Keep additions in sync with that file
// so Vue 3 + previous stay swappable while the migration finishes.
const IPS_OPTIONS = [
...
// === Routing-rule helpers (matches Nova_x-ui templateRule{Getter,Setter}) ==
...
function ruleSetter(outboundTag, property, data) {
  ...
  // previous single-write-then-filter behavior).
  const next = [];
  let inserted = false;
  for (const rule of props.templateSettings.routing.rules) {
    const matches =
...
function syncOutbound(tag, settings) {
  ...
  // pruned when none remain (previous syncRulesWithOutbound).
