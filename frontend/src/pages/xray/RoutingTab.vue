...
// modernised previous column layout. Each row is rendered as a single
// "lead value + N more" pill per criterion (matches the previous pill
// layout); full lists surface via tooltip on hover.
//
// Reorder uses up/down buttons in the action menu rather than the
// jQuery-Sortable drag handle the Nova_x-ui panel used — same effect,
...
// === Table data — match the previous routingRuleData shape ============
...
function onRuleConfirm(rule) {
  ...
  // object with only `type: "field"`. Match previous: skip the write
