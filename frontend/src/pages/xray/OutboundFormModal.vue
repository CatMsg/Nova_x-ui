...
// Structured outbound add/edit modal — mirrors the previous
// web/html/form/outbound.html. Covers every protocol + transport
// combination the Nova_x-ui panel exposes; the JSON tab still lets
...
// ============== Link → outbound ==============
// Mirrors the previous convertLink: dispatches into Outbound.fromLink,
// which handles vmess:// (base64 JSON), vless://, trojan://, ss://
// (param-link form), and hysteria(2)://. Anything else returns null
// from the model and we surface "Wrong Link!" the same as previous.
    ...
    <a-tabs v-if="outbound" v-model:active-key="activeKey">
      ...
      <a-tab-pane key="1" :tab="t('pages.xray.basicTemplate')">
        <a-form :colon="false" :label-col="{ md: { span: 8 } }" :wrapper-col="{ md: { span: 14 } }">
          ...
          <template v-if="isVMessOrVLess">
            ...
            <!-- Reverse-Sniffing — surfaced only when a reverse tag is set,
                 mirroring the previous form. Defaults populated by the model
          ...
          <template v-if="outbound.canEnableStream()">
            ...
            <!-- XHTTP — full parity with previous outbound form. The model
        ...
        <!-- Gated by canEnableStream() so TCP masks don't leak into
             ...
             (they don't have a stream config at all). Matches previous. -->
