...
export const sections = [
...

  {
    ...
    endpoints: [
      ...
      },
      {
        ...
        summary:
          "Return the URL(s) for one client on one inbound — the same string the Copy URL button copies in the panel UI. Supported protocols: vmess, vless, trojan, shadowsocks, hysteria, hysteria2. If streamSettings.externalProxy is set, returns one URL per external proxy. Protocols without a URL form (socks, http, mixed, wireguard, dokodemo-door, tunnel, tun) return an empty array.",
        params: [
          { name: 'id', in: 'path', type: 'number', desc: 'Inbound ID.' },
          { name: 'email', in: 'path', type: 'string', desc: 'Client email.' },
        ],
...

  {
    ...
    endpoints: [
      ...
      },
      {
        ...
        summary: 'Check whether a newer Nova_x-ui release is available on GitHub.',
      },
      {
        method: 'GET',
...

  {
    ...
    title: 'Nodes API',
    description:
      'Manage remote Nova_x-ui panels acting as nodes for a central panel. All endpoints under /panel/api/nodes.',
    endpoints: [
      {
        method: 'GET',
...

  {
    ...
    endpoints: [
      ...
      },
      {
        ...
        summary: 'Restart the entire Nova_x-ui process after a 3-second grace period. The connection drops immediately; the panel comes back online ~5-10 seconds later.',
      },
      {
        method: 'GET',
        path: '/panel/setting/getHy2ForwardBackend',
