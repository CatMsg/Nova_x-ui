...
// Remark model — previous stores it as a single string where index 0 is
...
// Nova_x-ui panel reloads on change so the Go side renders templates in
// the new language.
...
const hy2ForwardBackend = ref('loading');
...
async function loadHy2ForwardBackend() {
  const msg = await HttpUtil.get('/panel/setting/getHy2ForwardBackend');
  if (msg?.success && typeof msg.obj === 'string' && msg.obj.trim()) {
    hy2ForwardBackend.value = msg.obj.trim();
  } else {
    hy2ForwardBackend.value = 'none';
  }
}

const hy2BackendTagColor = computed(() => {
  switch (hy2ForwardBackend.value) {
    case 'ufw':
      return 'green';
    case 'nftables':
      return 'blue';
...
<template>
  <a-collapse default-active-key="1">
    <a-collapse-panel key="1" :header="t('pages.settings.panelSettings')">
      ...
      <SettingListItem paddings="small">
        <template #title>{{ t('pages.settings.hy2ForwardBackend') }}</template>
        <template #description>{{ t('pages.settings.hy2ForwardBackendDesc') }}</template>
        <template #control>
          <a-tag :color="hy2BackendTagColor" class="hy2-backend-tag">
            {{ hy2ForwardBackend }}
          </a-tag>
        </template>
      </SettingListItem>
