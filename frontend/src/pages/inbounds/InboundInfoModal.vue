...
// Nova_x-ui template did the same. The big v-if forks at the top decide
...
//   • Mixed/HTTP/Tunnel/Dokodemo/TUN → connection details only
//
...
const props = defineProps({
  ...
  // Sidecar config the Nova_x-ui panel keyed off `app.*`.
...
// IP-log state (matches the previous refresh / clear flow).
    ...
    <template v-if="dbInbound && inbound">
      <a-tabs v-model:active-key="activeTab">
        ...
        <a-tab-pane v-if="showClientTab" key="client" :tab="t('pages.inbounds.client')">
          <table class="info-table block">
            <tbody>
              ...
              <tr v-if="clientSettings.password || clientSettings.pass">
                <td>{{ t('password') }}</td>
                <td><a-tag class="info-large-tag">{{ clientSettings.password || clientSettings.pass }}</a-tag></td>
              </tr>
              <tr>
                <td>{{ t('status') }}</td>
                <td>
        ...
        <a-tab-pane key="inbound" :tab="t('pages.xray.rules.inbound')">
          ...
          <!-- Tunnel -->
          <dl v-if="dbInbound.isTunnel" class="info-list info-list-block">
            <div class="info-row">
            ...
            <div v-if="inbound.settings.portMap?.length" class="info-row info-row-stack">
              <dt>Port map</dt>
              <dd class="account-row">
                <a-tag v-for="(entry, idx) in inbound.settings.portMap" :key="idx" class="value-tag">
          ...
          <dl v-if="inbound.protocol === Protocols.TUN" class="info-list info-list-block">
            ...
            <div class="info-row">
              <dt>User level</dt>
              <dd><a-tag color="green">{{ inbound.settings.userLevel }}</a-tag></dd>
            </div>
          </dl>
