...
// Trojan / Shadowsocks-multi / Hysteria). The Nova_x-ui panel routes both
...
const isHttpLike = computed(() =>
  protocol.value === Protocols.HTTP
  || protocol.value === Protocols.SOCKS
  || protocol.value === Protocols.MIXED,
);
const accountName = computed({
  get: () => {
    if (!client.value) return '';
    return isHttpLike.value ? client.value.user : client.value.email;
  },
  set: (value) => {
    if (!client.value) return;
    if (isHttpLike.value) {
      client.value.user = value;
      client.value.email = value;
    } else {
      client.value.email = value;
    }
  },
...
function randomEmail() {
  if (client.value) accountName.value = RandomUtil.randomLowerAndNum(9);
}
function randomId() {
...
function randomPassword() {
  ...
  } else if (isHttpLike.value) {
    client.value.pass = RandomUtil.randomSeq(10);
  } else {
...
<template>
  <a-modal :open="open" :title="title"
    ...
    <a-form v-if="client && inbound" layout="horizontal" :colon="false" :label-col="{ sm: { span: 8 } }"
      ...
      <a-form-item>
        <template #label>
          {{ isHttpLike ? 'User' : t('pages.inbounds.email') }}
      ...
      <a-form-item v-else-if="isHttpLike">
        <template #label>
          {{ t('password') }}
          <SyncOutlined class="random-icon" @click="randomPassword" />
        </template>
