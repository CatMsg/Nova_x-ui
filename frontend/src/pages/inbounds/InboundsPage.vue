...
// === Export helpers — mirror previous txtModal call sites ==============
...
// `checkFallback` mirrors the previous helper: when an inbound listens
...
function findClientIndex(dbInbound, client) {
  ...
  const idx = clients.findIndex((c) => {
    ...
    switch (dbInbound.protocol) {
      ...
      case 'socks':
      case 'mixed':
        return c.user === client.user && c.email === client.email;
      default:
...
async function onToggleEnableClient({ dbInbound, client, next }) {
  // Mirror previous: clone the parsed inbound, flip enable on the matching
...
// Per-row destructive actions go through Modal.confirm (matches previous behavior).
...
.inbounds-page {
  ...
  background:
    ...
    radial-gradient(circle at 82% 10%, rgba(202, 226, 255, 0.32), transparent 28%),
...
.inbounds-page :deep(.ant-card-hoverable:hover) {
  border-color: rgba(123, 160, 224, 0.3);
  box-shadow: 0 22px 64px rgba(34, 51, 84, 0.12);
  transform: translateY(-1px);
}

.inbounds-page :deep(.ant-table),
.inbounds-page :deep(.ant-table-container),
.inbounds-page :deep(.ant-table-content) {
  background: rgba(255, 255, 255, 0.52);
  border-radius: 18px;
}

.inbounds-page :deep(.ant-table-thead > tr > th) {
  background: rgba(255, 255, 255, 0.84);
  border-bottom-color: rgba(122, 145, 176, 0.16);
}

.inbounds-page :deep(.ant-table-tbody > tr > td) {
  border-bottom-color: rgba(122, 145, 176, 0.12);
}

.inbounds-page :deep(.ant-btn-primary) {
  border-radius: 999px;
  box-shadow: 0 10px 22px rgba(24, 144, 255, 0.18);
}

.inbounds-page :deep(.ant-btn) {
  border-radius: 999px;
}
