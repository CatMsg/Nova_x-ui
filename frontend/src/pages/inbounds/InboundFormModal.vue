...
const hysteria2ServerPorts = computed({
  get: () => inbound.value?.stream?.hysteria?.serverPorts?.join(', ') || '',
  set: (value) => {
    if (!inbound.value?.stream?.hysteria) return;
    ...
    inbound.value.stream.hysteria.serverPorts = [...new Set(ports.map((item) => String(item)))];
  },
});
const hysteria2HopInterval = computed({
  get: () => {
    const raw = inbound.value?.stream?.hysteria?.hopInterval || '';
    const parsed = Number.parseInt(String(raw).replace(/s$/, ''), 10);
    return Number.isFinite(parsed) && parsed > 0 ? parsed : null;
  },
  set: (value) => {
    if (!inbound.value?.stream?.hysteria) return;
    const parsed = Number.parseInt(String(value ?? ''), 10);
    inbound.value.stream.hysteria.hopInterval = Number.isFinite(parsed) && parsed > 0 ? `${parsed}s` : '';
  },
});
...
// Date / GB bridges (previously used moment via _expiryTime; we go direct).
...
function onNetworkChange(next) {
  ...
  // Mirror previous streamNetworkChange: clear flow when TLS/Reality
...
// === SS method change tracks previous semantics =========================
  ...
  <a-modal :open="open" :title="title" :ok-text="okText" :cancel-text="t('close')" :confirm-loading="saving"
    ...
    <a-tabs v-if="inbound && dbForm" default-active-key="basic">
      ...
      <a-tab-pane key="protocol" :tab="t('pages.inbounds.protocol')">
        ...
        <!-- HTTP / Mixed accounts -->
        <a-form v-if="protocol === Protocols.HTTP || protocol === Protocols.MIXED || protocol === Protocols.SOCKS" :colon="false"
        ...
        <!-- Tunnel -->
        <a-form v-if="protocol === Protocols.TUNNEL || protocol === Protocols.DOKODEMO_DOOR" :colon="false" :label-col="{ sm: { span: 8 } }"
          :wrapper-col="{ sm: { span: 14 } }" class="mt-12">
          ...
          <a-form-item label="Port map">
            <a-button size="small" @click="inbound.settings.portMap.push({ name: '', value: '' })">
              <template #icon>
                <PlusOutlined />
              </template>
              Add
          ...
          <a-form-item v-if="inbound.settings.portMap.length > 0" :wrapper-col="{ span: 24 }">
            <a-input-group v-for="(entry, idx) in inbound.settings.portMap" :key="idx" compact class="mb-8">
              <a-input :style="{ width: '45%' }" v-model:value="entry.name" placeholder="Port">
                <template #addonBefore>{{ idx + 1 }}</template>
              </a-input>
              <a-input :style="{ width: '45%' }" v-model:value="entry.value" placeholder="Target host:port" />
              <a-button @click="inbound.settings.portMap.splice(idx, 1)">
                <template #icon>
                  <MinusOutlined />
                </template>
              </a-button>
        ...
        <a-form v-if="protocol === Protocols.TUN" :colon="false" :label-col="{ sm: { span: 8 } }"
          ...
          <a-form-item label="MTU">
            <a-input-number v-model:value="inbound.settings.mtu" :min="576" :max="9000" />
          </a-form-item>
          <a-form-item label="User level">
            <a-input-number v-model:value="inbound.settings.userLevel" :min="0" :max="65535" />
          </a-form-item>
        </a-form>
      ...
      <a-tab-pane v-if="canEnableStream" key="stream"
        ...
        <a-form :colon="false" :label-col="{ sm: { span: 8 } }" :wrapper-col="{ sm: { span: 14 } }">
          ...
          <template v-if="protocol === Protocols.HYSTERIA">
            <template v-if="inbound.stream.hysteria.version === 2">
              <a-form-item label="Server Ports">
                <a-input v-model:value="hysteria2ServerPorts" placeholder="2026, 2028, 2030" />
              </a-form-item>
              <a-form-item label="Hop Interval (s)">
                <a-input-number v-model:value="hysteria2HopInterval" :min="0" :step="1" placeholder="5" />
              </a-form-item>
            </template>
