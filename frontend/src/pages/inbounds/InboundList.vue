...
// Mirrors the previous logic: when searching, keep inbounds that match
...
// as previous: SS single-user inbounds and WireGuard inbounds expose
    ...
    <a-space direction="vertical" :style="{ width: '100%' }">
      ...
      <a-table v-else :columns="columns" :data-source="sortedInbounds" :row-key="(r) => r.id"
        ...
        <!-- Per-inbound client list, expanded by clicking the row's
             ...
             non-multi-user inbounds (matches previous behavior). -->
 ...
 * so the previous table-cell tightening rules went away. */
.inbound-cards {
  display: flex;
