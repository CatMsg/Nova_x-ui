...
// Bulk-add up to 500 clients in one go. The Nova_x-ui panel offers five
...
const isHttpLike = computed(() =>
  inbound.value?.protocol === Protocols.HTTP
  || inbound.value?.protocol === Protocols.SOCKS
  || inbound.value?.protocol === Protocols.MIXED,
);
...
watch(() => props.open, (next) => {
  ...
  // stateless between sessions (previous resets on .show()).
  form.emailMethod = 0;
  form.firstNum = 1;
  form.lastNum = 1;
  form.emailPrefix = '';
...
function buildClients() {
  ...
  for (let i = start; i < end; i++) {
    ...
    if (isHttpLike.value) c.user = c.email;
    ...
    // Use the client's totalGB setter (ms epoch and bytes already handled
